...
class BaseModel(MetaModel, nn.Module):
    ...
    def __init__(self, cfgs, training):
        ...
        if training:
            ...
            if 'data_cfg_mevid' in cfgs:
                self.train_loader2 = self.get_loader(
                cfgs['data_cfg_mevid'], train=True)
        if not training or self.engine_cfg['with_test']:
            if 'data_cfg_mevid' in cfgs:
                self.test_loader = self.get_loader(cfgs['data_cfg_mevid'], train=False)
            else:
    ...
    def get_loader(self, data_cfg, train=True):
        ...
        # ---- REPLACED: DataSet → build_dataset ----
