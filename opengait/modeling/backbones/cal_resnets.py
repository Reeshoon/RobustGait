...
from .cal_utils import c3d_blocks, inflate, nonlocal_blocks, pooling
...
class ResNet503D(nn.Module):
    def __init__(self, config, block, c3d_idx, nl_idx, **kwargs):
        super().__init__()
        self.block = block
        self.temperature = config["AP3D"]["TEMPERATURE"]
        self.contrastive_att = config["AP3D"]["CONTRACTIVE_ATT"]
...
def C2DResNet50(config, **kwargs):
    ...
    return ResNet503D(config, c3d_blocks.APP3DA, c3d_idx, nl_idx, **kwargs)