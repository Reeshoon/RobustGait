...
def compute_ap_cmc_MEVID(index, good_index, junk_index):
    ap = 0
...
def evaluate_mevid(distmat, q_pids, g_pids, q_camids, g_camids):
    num_q, num_g = distmat.shape
    ...
    for i in range(num_q):
        ...
        ap_tmp, CMC_tmp = compute_ap_cmc_MEVID(index[i], good_index, junk_index)
        if CMC_tmp[0]==1:
            num_r1 += 1
        CMC = CMC + CMC_tmp
        AP += ap_tmp
